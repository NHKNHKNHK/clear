...
Server如果在查询缓存中发现了这条SQL语句，就直接将结果返回给客户端；如果没有，就进入解析器阶段。
...
需要说明的是，在 MySQL 中的查询缓存，不是缓存查询计划，而是查询对应的结果。这就意味着查询匹配的`鲁棒性大大降低`，只有`相同的查询操作才会命中查询缓存`。两个查询请求在任何字符上的不同（例如：空格、注释、大小写），都会导致缓存不会命中。因此 MySQL 的查询缓存命中率不高。
...
一般建议在静态表里使用查询缓存，**静态表**是指极少更新的表，如系统配置表、字典表，这类表的查询适合使用查询缓存。MySQL 提供了 “`按需使用`” 的方式，可将 `my.cnf` 参数 `query_cache_type` 设置成 `DEMAND`，代表当 SQL 语句中有 `SQL_CACHE` 关键词时才缓存。例如：
...
这样默认 SQL 语句不使用查询缓存，对于确定要使用查询缓存的语句，可用 `SQL_CACHE` 显式指定，如：
...
-- mysql8中查询不到相关变量，进而验证了mysql8已将查询缓存移除
```
...
- `Qcache_free_blocks`：表示查询缓存中剩余的 blocks 数量，若该值较大，说明查询缓存中**内存碎片**过多，可能需在一定时间内整理。
- `Qcache_free_memory`：查询缓存的内存大小，通过该参数可清晰知晓当前系统查询内存是否够用，DBA 可根据实际情况调整。
- `Qcache_hits`：表示命中缓存的次数，可通过该值验证查询缓存效果，数字越大，缓存效果越理想。
- `Qcache_inserts`：表示未命中缓存后插入的次数，即新来的 SQL 请求在缓存中未找到，执行查询处理后将结果插入查询缓存的次数。该次数越多，说明查询缓存应用较少，效果不理想（系统刚启动时查询缓存为空属于正常情况）。
- `Qcache_lowmem_prunes`：记录因内存不足而被移除出查询缓存的查询条数，可通过该值适当调整缓存大小。
- `Qcache_not_cached`：表示因 `query_cache_type` 设置而未被缓存的查询数量。
...
如果SQL语句正确，则会生成一个语法树。例如：

![MySQL语法分析](./assets/MySQL语法分析.png)

下面是SQL词法分析的过程步骤：

![MySQL词法分析](./assets/MySQL词法分析.png)

至此，解析器的工作任务也就基本完成了。接下来进入到优化器。
...
> 如果你还有一些疑问，比如优化器是怎么选择索引的，有没有可能选择错等。就需要了解索引优化的细节了，[索引优化与查询优化](./索引优化与查询优化.md)
...
`profiling=0` 代表关闭，`profiling=1` 代表开启：
