...
在MySQL中，如果进行了索引合并，执行计划中会明确显示type为`index_merge`，key 中会列出用到的索引列表，并且在 extra 中会提示具体用了哪种索引合并策略：
...
```sql
SELECT * FROM Users WHERE age > 23 AND city = 'Shen Zhen';
```

> 需要说明的是，范围条件右边的列索引失效，如上`age > 23`后面的`city = 'Shen Zhen'`理论上是无法利用索引的，但是有了索引合并就可以
>
> 具体查看：[索引失效案例](./索引优化与查询优化#range-condition-right-column-index-invalidation)

**使用交集（Intersection）索引合并**：

- 数据库系统会首先使用`city`索引找到所有在Shen Zhen的用户。
- 然后使用`age`索引找到所有年龄大于23的用户。
- 最后，系统将这两个索引的结果做交集运算，从而得到同时满足这两个条件的用户列表。

**使用联合（Union）索引合并**：

- 数据库系统会分别使用`age`索引查找所有年龄大于23的用户，和使用`city`索引查找所有住在Shen Zhen的用户。
