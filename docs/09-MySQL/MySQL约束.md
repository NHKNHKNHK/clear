...
- **表级约束**：可以作用在多个列上，不与列一起，而是单独定义
  - 在表中所有字段声明完成之后，通过`CONSTRAINT 约束名`的方式来声明的约束

| | 声明位置 | 支持的约束类型 | 是否可以起约束名 |
| --- | --- | --- | --- |
| 列级约束 | 列的后面 | 语法都支持，但外键没有效果 | 不可以 |
| 表级约束 | 所有列的下面 | 默认和非空不支持，其他支持 | 可以（但主键没有效果）|
...
create table 表名称(
  字段名  数据类型,
  字段名  数据类型  unique, -- 后面直接跟关键字，约束名为字段名
  字段名  数据类型  unique key,
  字段名  数据类型
);
...
create table 表名称(
  ...
  字段名  数据类型,  
  字段名  数据类型,
  [constraint 约束名] unique key(字段名) -- 添加唯一约束，可以指定约束名
);
```
...
### MySQL 8.0新特性—自增变量的持久化 {#mysql8-auto-increment-persist}

在MySQL 8.0之前，InnoDB 存储引擎的自增主键（`AUTO_INCREMENT`））计数器仅在内存中维护，未持久化到磁盘。这导致：

- 数据库重启后，自增计数器会重置为 `max(主键值) + 1`；
- 若重启前存在「删除主键记录」的操作，重启后插入数据会重用已删除的主键值，引发主键冲突或业务逻辑异常（如依赖主键连续性的场景）。
...
从结果可以看出，新插入的0值分配的是主键值是4，按照重启前的操作逻辑，此处应该分配6。出现上述结果的主要原因是自增主键没有持久化。
...
<span style="color: var(----alibaba-qiangzhi-text-color);">【强制】</span>不得使用外键与级联，一切外键概念必须在应用层解决。

<span style="color: var(--alibaba-shuoming-text-color);">说明：</span>（概念解释）学生表中的 student_id 是主键，那么成绩表中的 student_id 则为外键。如果更新学生表中的 student_id，同时触发成绩表中的 student_id 更新，即为级联更新。外键与级联更新适用于`单机低并发`，不适合`分布式`、`高并发集群`；级联更新是强阻塞，存在数据库`更新风暴`的风险；外键影响数据库的`插入速度`。
