...
>   个人理解：就有点像我们不进入房间，在房间门口就知道房间里有人了；在数据库层面上，就是不进行行扫描或页扫描就知道已经表中已经存在锁了（可能不太准确，理解大致意思即可）
...
- 如果事务想要获取数据表中某些记录的`共享锁`，数据库就会在数据表上自动添加`意向共享锁`
- 如果事务想要获取数据表中某些记录的`排它锁`，数据库就会在数据表上自动添加`意向排他锁`
