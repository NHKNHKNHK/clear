...
需要说明的是，MVCC 没有正式的标准，在不同的 DBMS 中 MVCC 的实现方式可能是不同的，也不是普遍使用的（大家可以参考相关的 DBMS 文档）。

下面我们所说的是 InnoDB 中 MVCC 的实现机制（MySQL 其它的存储引擎并不支持它）。
...
我们知道事务有 4 个隔离级别，可能存在三种并发问题，下图是SQL标准中并发问题以及事务隔离级别关系的图解：
...
每次对记录进行改动，都会记录一条 undo 日志，每条 undo 日志都有一个`roll_pointer`属性，可以将这些 undo 日志连起来，串成一个链表，如下：
...
需要说明的是，此处的`INSERT`操作对应的 undo 日志没有`roll_pointer`属性，因为该记录并没有更早的版本
