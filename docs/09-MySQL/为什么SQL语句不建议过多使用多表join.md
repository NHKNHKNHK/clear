...
每个join操作都需要对两个或多个表进行连接操作，这个操作需要消耗大量的计算资源和时间。如果join太多，会导致SQL的执行效率下降。特别是在数据量大的时候。因为join的底层使用的是嵌套循环的方式，如果join的表过多，时间复杂度会指数级上升。
...
:::tip
关于多表查询分类，可以查看：[多表查询分类](./多表查询分类.md)
:::
...
当Inner Loop（内循环）的表用到字段有索引的话，可以用到索引进行查询数据，因为索引是B+树的，复杂度可以近似认为是 N*logM
