...
他只是将查询到的结构进行计算，而WHERE条件执行时机在SELECT语句执行之前，所以WHERE条件中的计算是先于索引的，所以索引是生效的。
...
### 范围条件右边的列索引失效 #{range-condition-right-column-index-invalidation}
...
以上的索引失效案例都是一般场景下的索引失效情况

对于某些特殊场景，以上情况不一定成立，比如：

- 不等于(!=或<>)索引失效
- like以通配符%开头索引失效

在一些特殊情况下，查询优化器基于成本考虑也会选择使用索引，具体查看：[什么是覆盖索引](#covering-index)
...
> [MySQL用了函数一定会索引失效吗](./MySQL用了函数一定会索引失效吗.md)
...
有一个常见又非常头疼的问题就是`limit 2000000,10`，此时需要 MySQL 排序前 2000010 记录，仅仅返回 `2000000 - 2000010` 的记录，其他记录丢弃，查询排序的代价非常大。（典型的深分页问题：[todo]）
...
>（假设存在zhangsh1234@xxx.com、zhangssxyz@xxx.com、zhangsy1998@aaa.com、zhangszhzsz@aaa.com
等数据）
...
Q：不太理解哪种情况下应该使用 EXISTS，哪种情况应该用 IN。选择的标准是看能否使用表的索引吗？

A：索引是个前提，其实选择与否还会要看表的大小。你可以将选择的标准理解为`小表驱动大表`。在这种方式下效率是最高的。
...
Q：在 MySQL 中统计数据表的行数，可以使用三种方式：`SELECT COUNT(*)`、`SELECT COUNT(1)` 和 `SELECT COUNT(具体字段)`，使用这三者之间的查询效率是怎样的？

A：前提：如果你要统计的是某个字段的非空数据行数，则另当别论，毕竟比较执行效率的前提是结果一样才可以。
...
> 更详细的介绍可以查看：[MySQL中count(*)、count(1)和count(字段名)有什么区别？](./MySQL中countx、count1和count字段名有什么区别？.md)
