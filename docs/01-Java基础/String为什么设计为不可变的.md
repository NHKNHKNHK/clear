...
```
would use an immutable whenever I can.
```
...
- 在JavaScript中，建议能使用`const`的地方就不使用`let`，因为这样可以避免无意中修改变量。
- 在Scala中，建议优先使用`val`而不是`var`

放在Java中也是如此，我们开发中，也应该优先考虑使用不可变对象。
...
对于这个例子，s1和s2都表示"abcd"，所以他们会指向字符串池中的同一个字符串对象

但是，之所以可以这么做，主要是因为字符串的不变性。试想一下，如果字符串是可变的，我们一旦修改了s1的内容，那必然导致s2的内容也被动的改变了，这显然不是我们想看到的。
...
> [String对象真的不可变吗](./String对象真的不可变吗.md)