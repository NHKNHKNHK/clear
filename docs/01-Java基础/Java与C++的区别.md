...
> [如何理解面向对象和面向过程](./如何理解面向对象和面向过程.md)
...
> 有人说Java是半编译、半解释型语言。Java 编译器(javac)先将java源程序编译成Java字节码(.class)，JVM负责解释执行字节码文件。
>
> 那么Java到底是编译型语言还是解释型语言？
>
> 具体查看：[Java是编译型还是解释型](../03-JVM篇/Java是编译型还是解释型.md)
...
> [Java是如何实现的平台无关？](../03-JVM篇/Java是如何实现的平台无关.md)

- Java是**自动内存管理和垃圾回收**的，C++需要手动内存管理，支持析构函数，Java没有析构函数的概念。

> [JVM中有哪些垃圾回收算法](../03-JVM篇/JVM中有哪些垃圾回收算法.md)

- C++支持指针，引用，传值调用。**Java只有值传递**。

> [Java中方法参数是值传递还是引用传递](./Java中方法参数是值传递还是引用传递.md)
