...
在 JavaEE/Android 中注解占据了更重要的角色，例如用来配置应用程序的任何切面，**代替** JavaEE 旧版中所遗留的**繁冗代码**和 **XML 配置**等。

未来的开发模式都是基于注解的。如JPA 是基于注解的，Spring2.5 以上都是基于注解的，Hibernate3.x 以后也是基于注解的，Struts2 有一部分也是基于注解的了。

因此，**注解是一种趋势**，一定程度上可以说：**框架 = 注解 + 反射 + 设计模式**
...
public class AnnotationTest {
    public static void main(String[] args) {
        ...
        // 该变量定义了，但是还未使用，就会出现警告，可以使用 @SuppressWarnings 标记来屏蔽警告
...
class Person {
    ...
    // @Deprecated 表示已经过时，不推荐使用，但是还可以使用
    @Deprecated
...
- 如：`TYPE`、`METHOD`、`CONSTRUCTOR`、`PACKAGE`...
...
- 需要注意的是，唯有 `RUNTIME` 阶段才能被反射读取到

（3）`@Documented`：表明这个注解应该被 javadoc 工具记录

（4）`@Inherited`：允许子类继承父类中的注解。默认情况下，注解不会被继承
...
- Annotation 的成员在 Annotation 定义中以无参数有返回值的抽象方法的形式来声明，我们又称为**配置参数**。
  - 返回值类型只能是八种基本数据类型、String 类型、Class类型、enum 类型、Annotation 类型、以上所有类型的数组
