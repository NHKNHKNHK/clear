...
指定被修饰的注解的生命周期，即注解在源代码、编译时还是运行时保留。它有三个可选的枚举值：`SOURCE`、`CLASS`和`RUNTIME`。默认为`CLASS`。
...
指示被该注解修饰的注解是否可以被继承。**默认情况下，注解不会被继承**，即子类不会继承父类的注解。但如果将一个注解用`@Inherited`修饰，那么它就可以被子类继承。
